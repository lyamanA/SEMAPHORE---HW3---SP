    ...
    public partial class Form1 : Form
    {
        ...
        private void MoveToWorking()
        {
            if (listBox_WaitingThreads.Items.Count > 0 && listBox_WorkingThreads.Items.Count < numericUpDown_PlacesinSemaphore.Value)
            {
                ...
                // новый поток дл€ выполнени€ работы
        ...
        private void numericUpDown_PlacesinSemaphore_ValueChanged(object sender, EventArgs e)
        {
            ...
            // ѕровер€ем, чтобы значение было больше нул€
            if (newValue > 0)
            {
            ...
            {
                // ≈сли значение равно нулю или меньше, устанавливаем минимальное значение на 1
                MessageBox.Show(" оличество мест в семафоре должно быть больше нул€.", "ќшибка", MessageBoxButtons.OK, MessageBoxIcon.Error);
